...
import openmm as mm
from atom_openmm.gibbs_sampling import pairwise_independence_sampling
from atom_openmm.ommreplica import OMMReplicaATM
...
# Override the integrator to use the LangevinMiddleIntegrator
def set_integrator(
    self,
    temperature,
    frictionCoeff,
...
OMMSystemRBFE.set_integrator = set_integrator
        ...
        with Timer(self.logger.info, "ATM setup"):
            ...
            with Timer(self.logger.info, "create system"):
                ...
                ommsystem = OMMSystemRBFE(
                    self.basename, self.config, pdbtopfile, systemfile, self.logger
                )
            ...
            with Timer(self.logger.info, "create replicas"):
                ...
                for i in range(self.nreplicas):
                    replica = OMMReplicaATM(
                        i, self.basename, self.worker, self.logger, self.config
                    )
                        ...
                        with TerminationGuard():
                            with Timer(
                                ...
                                for irepl, replica in enumerate(self.replicas):
                                    ...
                                    replica.save_out(output_data[irepl])
...
def rbfe_production(config_file=None):
    from atom_openmm.acellera.utils import set_directory
    ...
    with set_directory(Path(config_file).parent):
        rx = OpenmmJobRBFE(os.path.basename(os.path.abspath(config_file)))
        rx.setupJob()
        rx.scheduleJobs()
